...
namespace TouchPanel.Helpers
{
    public static class PCLHelper
    {
        ...
        public async static Task<bool> IsFileExistAsync(this string fileName, IFolder rootFolder = null)
        {
            ...
            try
            {
                ExistenceCheckResult folderexist = folder.CheckExistsAsync(fileName).Result;
        ...
        public static string ReadAllTextAsync(this string fileName, IFolder rootFolder = null)
        {
            string content = "";
            IFolder folder = rootFolder ?? FileSystem.Current.LocalStorage;
            bool exist = fileName.IsFileExistAsync(folder).Result;
            if (exist == true)
            {
